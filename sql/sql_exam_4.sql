...
​select e.ename "사원", e.empno "사원번호", m.ename "관리자"
from emp e, emp m
where e.mgr = m.empno
order by e.mgr
;
...
​
select e.ename, nvl(e.empno,0)empNo, e.job ,e.mgr, m.ename
from emp e, emp m
where e.mgr = m.empno(+)
order by e.empno
;
...
​select s.ename, s.deptno, e. ename
from emp e, emp s
...
select m.ename"mgerName", m.hiredate"mgrHiredate", e.ename"empName", e.hiredate"empHiredate"
from emp m, emp e
where e.mgr = m.empno
