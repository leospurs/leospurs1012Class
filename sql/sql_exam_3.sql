...
​select ename, substr(hiredate,1,5) as "입사 년/월" from emp;
...
​select ename, substr(hiredate,4,5) from emp;
...
​select trunc(sysdate - to_date('2021/01/01', 'YYYY/MM/DD')) as "결과값" from dual;
...
select max(sal) - min(sal) as "최고/최저 급여 차액" from emp;
​
