...
-- 사원 테이블과 유사한 구조의 
-- 사원번호, 사원이름, 급여 3개의 칼럼으로 구성된 EMP01을 생성해 봅시다.
desc emp;
...
-- 컬럼의 변경 : alter table {테이블 이름} modify (칼럼정의);

-- 직급(JOB) 칼럼을 최대 30 글자까지 저장할 수 있게 변경해 보도록 하자.
alter table emp01
modify (job varchar2(30) not null)
;


-- 컬럼의 삭제 : alter table {테이블 이름} drop column 컬럼 이름

-- EMP01 테이블의직급 칼럼을 삭제해 봅시다.
alter table EMP01 
drop column job;
