...
public class SmartPhone {
	...
	// Contact 클래스의 인스턴스 10개를 저장할 수 있는 배열을 정의합시다.
	// 배열에 인스턴스를 저장하고, 수정하고, 삭제, 저장된 데이터의 리스트를 출력하는 메소드를 정의합니다.
	
	private Contact[] contacts;	// Contact타입의 인스턴스를 저장하는 배열 선언
	private int cnt; // 저장된 Contact타입의 인스턴수 개수, 배열의 index로 사용
	...
	// 배열의 요소를 추가하는 메소드 : 참조값을 전달 받아 배열에 추가하는 기능
	...
	// 이름(문자열)을 전달 받아서 배열의 각 요소의 이름과 비교, 같은 요소가 있으면 해당 index를 변환
	private int searchIndex(String name) {
		
		int index = -1;
		
		// 배열의 요소를 반복해서 참조하고 이름과 비교 -> 찾으면 반복 종료
		for(int i = 0; i < cnt; i++) {
	...
	public void editContact() {
		...
		System.out.println("수정하고자하는 데이터의 이름을 입력하세요.");
		String name = sc.nextLine();
